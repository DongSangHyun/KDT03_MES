...
namespace GNU_MES
{
    ...
    static class Program
    {
        ...
        static void Main()
        {
            ...
            if (GNU_MES.Program.CheckMultiProcess()) return;

            // 프로그램 전체에서 사용되어지는 Style 파일을 정의
            Microsoft.Win32.RegistryKey rkey = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(@"DC_MES\UI");
            if (rkey != null)
            {
            ...
            // 프로그램 실행
            GNU_MES.Program.RunApplication(new string[] { userid, userName });
        }
